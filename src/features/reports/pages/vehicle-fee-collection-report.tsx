...
import { Route } from '@/routes/_protected/reports/vehicle-fee-collection'
...
export function VehicleFeeCollectionReportPage() {
  ...
  const navigate = useNavigate({ from: '/reports/vehicle-fee-collection' })
  ...
  useEffect(() => {
    ...
    setInitialized(true)
    if (!search.from && !search.to && !search.fiscal_year_id && !search.search) {
  ...
  useEffect(() => {
    setDraft({ from: search.from || '', to: search.to || '', fiscal_year_id: search.fiscal_year_id, search: search.search || '' })
  }, [search.from, search.to, search.fiscal_year_id, search.search])

  const appliedFilters: ReportFilters = { from: search.from || undefined, to: search.to || undefined, fiscal_year_id: search.fiscal_year_id || undefined, search: search.search || undefined }
  ...
  const handleApplyFilters = () => {
    navigate({ search: { from: draft.from || undefined, to: draft.to || undefined, fiscal_year_id: draft.fiscal_year_id || undefined, search: draft.search || undefined }, replace: true })
  }
